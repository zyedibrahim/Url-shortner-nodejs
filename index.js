...
if(err){
  ...
  app.get('/users/reset-password/:id/:token/:random', async function(request,response){

const{id,token,random} =request.params;
...
const verify = Jwt.verify(token,secret)
  ...
  response.status(400).send({"message":"not verifed" })
}

  } )

  app.post('/users/reset-password/', async function(req, res) {
    const {id,token,random} = req.query;
...
if(olduser.random === random){
  const encryptpassword = await generateHashedPassword(password)
  const updateddata = await client
    .db('userdata')
    .collection('userdetails')
    .updateOne({ _id: new ObjectId(id) }, { $set: {password:encryptpassword,random:""} });
  
    res.status(200).send({"message": "succefully sented",updateddata })
  ...
  // console.log(encryptpassword,"this is newpass")
  
    // const updateddata = await client
    // .db('userdata')
    // .collection('userdetails')
    // .updateOne({ _id: new ObjectId(id) }, { $set: {password:encryptpassword}});
